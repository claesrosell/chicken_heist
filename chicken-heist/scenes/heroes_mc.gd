...
func calculate_steering(delta: float) -> void:
	...
	if Input.is_action_pressed(GameManager.foxy_controls.mc_handbrake):
...
func check_skid_marks() -> void:
	...
	if Input.is_action_pressed(GameManager.foxy_controls.mc_handbrake) and velocity.length() > 50:
		is_drifting = true
