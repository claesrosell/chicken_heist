...
@export var time_to_hen : float = 3
var time_to_hen_left : float = time_to_hen
...
func _process(delta: float) -> void:
	time_to_hen_left -= delta

	if hens.get_child_count() < target_hen_count && time_to_hen_left <= 0:
